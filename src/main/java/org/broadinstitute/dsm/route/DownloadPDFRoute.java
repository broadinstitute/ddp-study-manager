...
public class DownloadPDFRoute extends RequestHandler {
    ...
    private static final String SQL_SELECT_REALM_FOR_PARTICIPANT = "SELECT inst.instance_name, inst.base_url, inst.ddp_instance_id, inst.mr_attention_flag_d, " +
            "inst.tissue_attention_flag_d, inst.es_participant_index, inst.es_activity_definition_index, nst.es_users_index, inst.auth0_token, inst.notification_recipients, inst.migrated_ddp, inst.billing_reference, part.ddp_participant_id, (SELECT count(role.name) " +
